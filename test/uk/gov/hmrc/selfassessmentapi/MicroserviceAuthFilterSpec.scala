...
class MicroserviceAuthFilterSpec extends UnitSpec with Matchers {
  ...
  "MicroserviceAuthFilter" should {
    ...
    "extract resource that builds valid auth url" in {
      val resource = filter.extractResource(s"/$nino/employments", HttpVerb("GET"), AuthConfig(pattern = "/(\\w+)/.*".r, confidenceLevel = L0))
      resource.get.buildUrl("http://authhost.com/auth", AuthRequestParameters(L0)) shouldBe s"http://authhost.com/auth/authorise/read/paye/$nino?confidenceLevel=0"
    }
  }
}
